        ...
        for i in range(len(json.get("exchangeRate"))):
            if json.get("exchangeRate")[i].get('baseCurrency') == currency_a and json.get("exchangeRate")[i].get('currency')  == currency_b:
                ...
                if curency_curse == 'НБУ':
                    return f'Курс НБУ на {currency_date} у випадку конвертації {currency_a} в {currency_b} - курс {rate_buyNB} : {rate_sellNB}'
                else:
                    return f'Курс Приватбанку на {currency_date} у випадку конвертації {currency_a} в {currency_b} - для покупки {rate_buy} та для продажу {rate_sell}'
    return f"Помилка api {response.status_code}: {json.get('errorDescription')}"
